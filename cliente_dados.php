...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $novaDataNascimento = $_POST["txtDataNascimento"];

    // Validações (adicione suas validações personalizadas aqui)
    if (empty($novoNome) || empty($novoCPF) || empty($novaDataNascimento)) {
        $erro = "Todos os campos são obrigatórios.";
    } else {
        // Atualize os dados no banco de dados
        $stmt = $conexao->prepare("UPDATE usuarios SET nome = ?, cpf = ?, data_nascimento = ? WHERE id = ?");
        $stmt->bind_param("sssi", $novoNome, $novoCPF, $novaDataNascimento, $usuarioId);
        if ($stmt->execute()) {
            $sucesso = "Dados atualizados com sucesso!";

            // Atualiza os dados na sessão após a alteração
...
$stmt = $conexao->prepare("SELECT nome, cpf, data_nascimento FROM usuarios WHERE id = ?");
                    ...
                    <div class="col-8">
                        <form method="POST" action=""> 
                            <div class="form-floating mb-3">
                                <input class="form-control" type="text" id="txtNome" name="txtNome" placeholder=" " value="<?php echo $usuario['nome']; ?>" autofocus />
                                <label for="txtNome">Nome</label>
                            </div>
                            <div class="form-floating mb-3 col-md-6 col-xl-4">
                                <input class="form-control" type="text" id="txtCPF" name="txtCPF" placeholder=" " value="<?php echo $usuario['cpf']; ?>" />
                                <label for="txtCPF">CPF</label>
                            </div>
                            <div class="form-floating mb-3 col-md-6 col-xl-4">
                                <input class="form-control" type="date" id="txtDataNascimento" name="txtDataNascimento" placeholder=" " value="<?php echo $usuario['data_nascimento']; ?>" />
